...
% August 2023, revision 3
% notices-ed@amsjournals.org  rlevine@sdsu.edu
...
When matrices $X$ and $Y$ commute (i.e., $XY=YX$), we have $G(X,Y)=\sqrt{XY}$.\correction{\st{,
 and when $Y^{-1}X$ or $X^{-1}Y$  is symmetric positive-definite, we have $G(X,Y)=Y(Y^{-1}X)^{\frac{1}{2}}$ or $G(X,Y)=X(X^{-1}Y)^{\frac{1}{2}}$, respectively.}}
