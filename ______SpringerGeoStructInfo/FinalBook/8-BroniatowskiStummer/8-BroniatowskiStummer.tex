...
adjacent artificial intelligence (AI). 
...
for tasks in data analytics such as statistics, machine learning, and 
...
as well as more flexible hybrids $d_{m_{1},m_{2},m_{3}}^{(5)}(p,q) := m_{3} \cdot 
...
\ identity of indiscernibles\footnote{see e.g. Weller-Fahy et al.~\cite{Wel:15}}),

\end{enumerate}
and such $d(\cdot,\cdot)$ is then called a \textit{divergence} (or disparity, contrast function). Loosely speaking,  
...
D_{\phi}^{(j)}(P,Q)
:=
\sum_{x \in \mathcal{X}} d_{\phi}^{(j)}(p(x),q(x)) \cdot \lambda(x)
\ \ \textrm{or} \ \ 
D_{\phi}^{(j)}(P,Q) := \int\displaylimits_{\mathcal{X}} d_{\phi}^{(j)}(p(x),q(x)) \, \mathrm{d}\lambda(x)
$$
...
respectively aggregated dissimilarities $D(\cdot, \cdot)$ between two functions 
...
see e.g. Amari~\cite{Ama:00}, Amari~\cite{Ama:16}, Ay et al.~\cite{Ay:17}.
Moreover, for recent connections between divergence-based information geometry and optimal transport
the reader is e.g. referred to Pal \& Wong~\cite{Pal:16,Pal:18}, Karakida \& Amari~\cite{Kar:17}, Amari et al.~\cite{Ama:18},
Peyre \& Cuturi~\cite{Pey:18}, and the 
literature
therein. 
...
$\omega \negthinspace \rightarrow \negthinspace d_{\phi}^{(j)}(p(x,\omega),q(x,\omega)), \  
\omega \negthinspace \rightarrow \negthinspace d_{m_{1}(x),m_{2}(x),m_{3}(x)}^{(5)}(p(x,\omega),q(x,\omega))$\ 
($x \in \mathcal{X}$)
as well as with the random-variable-type
``summarizing''
divergences\\
$\omega \negthinspace \rightarrow \negthinspace D_{\phi}^{(j)}(P(\omega),Q(\omega))
:=
...
as well as with\\
$\omega \negthinspace \rightarrow \negthinspace D_{\phi, M_{1}, M_{2}, M_{3}}^{(5)}(P(\omega),Q(\omega))
\negthinspace  := \negthinspace 
\sum_{x \in \mathcal{X}} \negthinspace { d_{m_{1}(x),m_{2}(x),m_{3}(x)}^{(5)}(p(x,\omega),q(x,\omega)) \cdot \lambda(x) }$,\\
resp.\
$\omega \negthinspace \negthinspace \rightarrow \negthinspace \negthinspace 
D_{\phi, M_{1}, M_{2}, M_{3}}^{(5)}\negthinspace(P(\omega),Q(\omega)) 
\negthinspace := \negthinspace
\int_{\mathcal{X}} \negthinspace
d_{m_{1}(x),m_{2}(x),m_{3}(x)}^{(5)}\negthinspace(p(x,\omega),q(x,\omega)) \, \mathrm{d}\lambda(x)$.
...
$q(x) \mathrm{d}x = \widetilde{Pr}[\{\omega \in \Omega \negthinspace : \negthinspace Y(\omega) \in [x,x + \mathrm{d}x[ \}$,
...
Correspondingly, the abo\-ve-mentioned procedure for the deterministic case
...
in Ki{\ss}linger \& Stummer~\cite{Kis:17b}: the basic idea is
to pick out two 
\footnote{
...
$data_{two}(\omega):= $\\
 $(data_{t_1}(\omega),\ldots, data_{t_{N_2}}(\omega))$,
...
\subsection*{Main (i.e. non-locally used) Notation and Symbols}

\vspace{-0.4cm}

{\scriptsize

\begin{longtable}[l]{p{2.75cm} l}
$\mathbb{R}$, $\mathbb{N}$, $\mathbb{R}^d$
& Set of real respectively integer numbers respectively $d$-dimensional vectors
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\Theta$, $\theta$ 
& Set of parameters, see p.~\pageref{notat:Theta}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbb{1}$ & Function with constant value 1
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\boldsymbol{1}_{A}(z) = \delta_{z}[A]$ &  Indicator function on the set $A$ evaluated at data point $z$, 
which
\\
...
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{F}$  &  System of admissible events/data-collections ($\sigma$-algebra) on $\mathcal{X}$
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\lambda$  &  Reference measure/integrator/summator, 
see p.~\pageref{notat:lambda} 
\& Subsec.~\ref{subsec.2.1verynew} on p.~\pageref{subsec.2.1verynew}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\lambda$-a.a. &   $\lambda$-almost all, see p.~\pageref{notat:lambdaaa}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\lambda_{L}$  &  Lebesgue measure (``Riemann-type'' integrator), see p.~\pageref{notat:lambdaL},
\& Subsec.~\ref{subsec.2.1verynew} 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\lambda_{\#}$ &  Counting measure (``classical summator''), see p.~\pageref{notat:lambdahash} 
\& Subsec.~\ref{subsec.2.1verynew} on p.~\pageref{subsec.2.1verynew}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$P \negthinspace := \negthinspace \big\{p(x)\big\}_{x \in \mathcal{X}}$ &   
Function from which the divergence/dissimilarity is measured from, see p.~\pageref{notat:Pfunc}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$Q \negthinspace := \negthinspace \big\{q(x)\big\}_{x \in \mathcal{X}}$ &   
Function to which the divergence/dissimilarity is measured to, see p.~\pageref{notat:Qfunc}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$M_{i} \negthinspace := \negthinspace \big\{m_{i}(x)\big\}_{x \in \mathcal{X}}$ &   
Scaling function ($i=1,2$) respec. aggregation function ($i=3$),
see p.~\pageref{brostu2:fo.def.1}, 
\\
& \eqref{brostu2:fo.def.1} and paragraph (I1) thereafter,
as well as  
Subsec.~\ref{subsec.2.3verynew} on p.~\pageref{subsec.2.3verynew}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$p(\cdot)$, $q(\cdot)$,
$m_{i}(\cdot)$, 
& Alternative representations of $P$, $Q$, $M_{i}$ 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$R \negthinspace := \negthinspace \big\{r(x)\big\}_{x \in \mathcal{X}}$ &   
Function used for the aggregation function $m_{3}(\cdot)$,
see Subs. 3.3.1 on p.~\pageref{notat:Subs.3.3.1}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$W_{i}\negthinspace 
$
& Connector function of the form $W_{i}\negthinspace := \negthinspace \big\{w_{i}(x,y,z)\big\}_{x,y,z \in \ldots}$,
for adaptive 
\\
&  scaling and aggregation functions $m_{i}(x) = w_{i}(x,p(x),q(x))$ ($i=1,2,3$),
\\
& see e.g. Assumption \ref{brostu2:assu.class1} on p.~\pageref{brostu2:assu.class1}
and Subsection 3.3.1.3 on p.~\pageref{notat:Subs.3.3.1.3}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{P}$, $\mathbbm{Q}$, $\mathbbm{M}_{i}$, $\mathbbm{W}_{i}$ &   
Functions with $\mathbbm{p}(x) \geq 0$, 
$\mathbbm{q}(x) \geq 0$, $\mathbbm{m}_{i}(x) \geq 0$, $\mathbbm{w}_{i}(x) \geq 0$
for $\lambda$-a.a. $x \in \mathcal{X}$ 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{Q}^{\chi} \negthinspace := \negthinspace 
\big\{\mathbbm{q}^{\chi}(x)\big\}_{x \in \mathcal{X}}$ &   
Function for the aggregation function $m_{3}(\cdot)$,
see Subs. \ref{subsec.4anew.2} on p.~\pageref{subsec.4anew.2},
\eqref{brostu2:fo.div.new0} 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{\overrightharp{P}}$,
$\mathbbm{\overrightharp{Q}}$ 
&  $\lambda$-probability density functions (incl. probability mass functions for
$\lambda=\lambda_{\#}$),
\\
& i.e. for which $\mathbbm{\overrightharp{p}}(x) \geq 0$, 
$\mathbbm{\overrightharp{q}}(x) \geq 0$ for $\lambda$-a.a. $x \in \mathcal{X}$
and $\int_{\mathcal{X}} \mathbbm{\overrightharp{p}}(x) \,  \mathrm{d}\lambda(x) = 1$,
\\
& see Remark \ref{brostu2:rem.40} on p.~\pageref{brostu2:rem.40}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{\overrightharp{Q}}_{\theta} := 
\big\{\mathbbm{\overrightharp{q}}_{\theta}(x)\big\}_{x \in \mathcal{X}}$
& $\lambda$-probab. density function which depends on a parameter $\theta \in \Theta$,
see p.~\pageref{notat:Qtheta} 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{R}\big(\frac{P}{M_{1}}\big)$ & Range (image) of 
the function $\big\{\frac{p(x)}{m_{1}(x)}\big\}_{x \in \mathcal{X}}$, see paragraph (I2)
on p.~\pageref{notat:I2}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{R}(Y_{1}, \ldots, Y_{N})$ & Range (image) of 
the random variables $Y_{1}, \ldots, Y_{N}$, see p.~\pageref{notat:rangeRV}
\\ 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\widetilde{\mathbbm{\overrightharp{Q}}}_{\theta} := 
\big\{\widetilde{\mathbbm{\overrightharp{q}}}_{\theta}(x)\big\}_{x \in \mathcal{X}}$
& 
$\lambda$-probab. density function (modification of $\mathbbm{\overrightharp{Q}}_{\theta}$)
defined by\\
& $\widetilde{\mathbbm{\overrightharp{q}}}_{\theta}(x)
:= \mathbbm{\overrightharp{q}}_{\theta}(x) \cdot (1 - \boldsymbol{1}_{\mathcal{R}(Y_{1}(\omega), \ldots, Y_{N}(\omega))}(x))$,
see p.~\pageref{notat:Qtilde} 
\\ 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\phi \negthinspace := \negthinspace \big\{\phi(t)\big\}_{t \in ]a,b[}$  &  
Divergence generator, a convex real-valued function on $]a,b[$, 
see p.~\pageref{brostu2:fo.def.1}, \eqref{brostu2:fo.def.1} 
\\
& and  paragraph (I2),
as well as Subsec.~\ref{subsec.2.2verynew} on p.~\pageref{subsec.2.2verynew}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\Phi(]a,b[)$; & Class of all such $\phi$, see paragraph (I2) on p.~\pageref{notat:I2}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\overline{\phi} := \negthinspace \big\{\phi(t)\big\}_{t \in [a,b]}$ &   
Continuous extension of $\phi$ on $[a,b]$, with $\overline{\phi}(t) = \phi(t)$ for all $t \in ]a,b[$,
see (I2)
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\phi_{+,c}^{\prime}(t)$ &  $c$-weighted mixture of left-hand and right-hand derivative of $\phi$ at t,
see (I2) 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\Phi_{C_{1}}(]a,b[)$ & Subclass of everywhere continuously differentiable $\phi$,
with \\
& derivative $\phi^{\prime}(t)$ (being equal to $\phi_{+,c}^{\prime}(t)$ for all $c \in [0,1]$),
see (I2) on p.~\pageref{notat:I2}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\phi_{\alpha}$ & $\alpha$-power-function type divergence generator, see \eqref{brostu2:fo.def.32}
on p.~\pageref{brostu2:fo.def.32}, \eqref{brostu2:fo.def.34a}, 
\eqref{brostu2:fo.def.120b}, \eqref{brostu2:fo.def.120d}
\\ 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\phi_{TV}$ & Generator of total variation distance, see \eqref{brostu2:fo.def.68a0}
on p.~\pageref{brostu2:fo.def.68a0}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\phi_{ie}$ & Divergence generator with interesting effects, see 
\eqref{brostu2:fo.876} on p.~\pageref{brostu2:fo.876} 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\psi_{\phi,c}$ &  Function given by 
$\psi_{\phi,c}(s,t) := \phi(s) - \phi(t) - \phi_{+,c}^{\prime}(t) \cdot (s-t) \geq 0$, see (I2)
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\overline{\psi_{\phi,c}}$  &   Bivariate extension of $\psi_{\phi,c}$, see (I2) on p.~\pageref{notat:I2}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\olint_{{\mathcal{X}}} \ldots$,  
$\olsum_{{\mathcal{X}}} \ldots$ &   Integral/sum over extension of integrand/summand $\ldots$,
see (I2) \& \eqref{brostu2:fo.def.5} on p.~\pageref{brostu2:fo.def.5}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$D^{c}_{\phi,M_{1},M_{2},\mathbbm{M}_{3},\lambda}(P,Q)$ &   
Divergence between two functions $P$ (scaled by $M_{1}$) and 
$Q$ (scaled by $M_{2}$),
\\
& generated by $\phi$ and weight $c$, and aggregated by $\mathbbm{M}_{3}$ and $\lambda$,
 see \eqref{brostu2:fo.def.1} on p.~\pageref{brostu2:fo.def.1}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$D_{\phi,M_{1},M_{2},\mathbbm{M}_{3},\lambda}(P,Q)$ &   As above, but with $\phi \in \Phi_{C_{1}}(]a,b[)$ and obsolete $c$, see 
 Subsec.~\ref{subsec.2.2verynew} on p.~\pageref{subsec.2.2verynew}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$D_{\lambda}(P,Q)$ & General $\lambda$-aggregated divergence, see  p.~\pageref{notat:Dlambda},
respectively pseudo-divergence, 
\\
& see Definition \ref{brostu2:def.pseudo}
on p.~\pageref{brostu2:def.pseudo}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$D_{\mathbbm{M},\lambda}(\mathbbm{\overrightharp{P}},\mathbbm{\overrightharp{Q}})$
& Pointwise decomposable pseudo-divergence, scaled by $\mathbbm{M}$
\\
& and aggregated by $\mathbbm{M}$ and $\lambda$,
see Subsec. \ref{subsec.4anew.5} on p.~\pageref{subsec.4anew.5}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
NN0, NN1 & Nonnegativity setup 0 respectively 1, see  p.~\pageref{notat:NN0} resp. p.~\pageref{notat:NN1}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathfrak{P}^{\mathbbm{R} \cdot \lambda}$, 
$\mathfrak{Q}^{\mathbbm{R} \cdot \lambda}$,
$\mathfrak{M}^{\mathbbm{R} \cdot \lambda}$
& Measures with $\lambda$-densities $p(\cdot) \cdot r(\cdot)$, $q(\cdot) \cdot r(\cdot)$, $m(\cdot) \cdot r(\cdot)$,
\\
& see Remark \ref{brostu2:rem.40} on p.~\pageref{brostu2:rem.40}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathfrak{\overrightharp{P}}^{\mathbb{1}\cdot \lambda}$, 
$\mathfrak{\overrightharp{Q}}^{\mathbb{1}\cdot \lambda}$
& Probability measures (distributions) with $\lambda$-densities $p(\cdot)$, $q(\cdot)$,
see Remark \ref{brostu2:rem.40}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{Q}_{\Theta}^{\lambda_{2}}$,
$\mathfrak{\overrightharp{Q}}^{\mathbb{1}\cdot 
\lambda_{2}}_{\theta }$
& Class of probability measures with $\lambda_{2}$-densities $q_{\theta}(\cdot)$
with parameter $\theta \in \Theta$, 
\\
& see p.~\pageref{notat:Qthetalamda2}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathfrak{\overrightharp{P}}_{N}^{emp}$, 
$\mathbbm{\overrightharp{P}}_{N}^{emp}$, $\mathbbm{\overrightharp{p}}_{N}^{emp}(\cdot)$
& Data-derived empirical (probability) distribution, 
and probability mass 
\\
&  function ($\lambda_{\#}$-density) thereof, 
see Remark \ref{brostu2:rem.40} on p.~\pageref{brostu2:rem.40}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathfrak{\overrightharp{P}}_{N}^{\overline{emp}(\omega)}$, 
$\mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)}$ 
& Data-derived ``extended'' empirical (probability) distribution,  and  
\\ 
& probability mass function thereof, 
see \eqref{brostu2:fo.def.pnempcont} on p.~\pageref{brostu2:fo.def.pnempcont}
and thereafter
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
DPD, CASD & Density-power divergences (see p.~\pageref{notat:DPD}), Csiszar-Ali-Silvey divergences (see p.~\pageref{notat:CASD})
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\ell i_{1}$, $\phi^{*}(0)$, $\ell i_{2}$, $\ell i_{3}$
&  Certain limits, see \eqref{brostu2:fo.def.45bthree}, \eqref{brostu2:fo.def.45bthreevar}, 
\eqref{brostu2:fo.def.45bfourchi}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{P} \perp \mathbbm{Q}$  &  The functions $\mathbbm{P}$, $\mathbbm{Q}$ are ``essentially different'', see 
\eqref{brostu2:fo.Sing.1} to \eqref{brostu2:fo.Sing.3}
and 
thereafter 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{P} \not\perp \mathbbm{Q}$ &  Negation of $\mathbbm{P} \perp \mathbbm{Q}$, see 
p.~\pageref{notat:nonsing}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{P} \sim \mathbbm{Q}$  &  The functions $\mathbbm{P}$, $\mathbbm{Q}$ are ``equivalent''
(concerning zeros), see 
\eqref{brostu2:fo.mesequ}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathbbm{P} \not \sim \mathbbm{Q}$  &  
Negation of $\mathbbm{P} \sim \mathbbm{Q}$, see 
p.~\pageref{notat:notsim}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\widehat{\theta}_{N,D_{\lambda_{2}}}$ &  
Minimum-divergence estimator (``approximator'')
of the true unknown 
\\
& parameter $\theta_{0}$, based on $N$ data observations, see \eqref{15} on p.~\pageref{15}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\widehat{\theta}_{N,D_{\lambda_{\#}}}$, $\widehat{\theta}_{N,D_{\lambda}}$  &   
Certain minimum-divergence estimators, see \eqref{brostu2:fo.829}, \eqref{15enc}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\widehat{\theta}_{N \negthinspace , decD_{\lambda}}$\negthinspace, \negthinspace 
$\widehat{\theta}_{N\negthinspace , decD_{\mathbbm{\overrightharp{Q}}_{\tau},\lambda}}$ \negthinspace 
\negthinspace   &   
Certain minimum-divergence estimators, see \eqref{c1new}, \eqref{brostu2:fo.519}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\widehat{\theta}_{N,sup\mathcal{D}_{\phi,\lambda}}$ &     
Certain minimum-divergence estimator, see \eqref{brostu2:fo.941} 
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{P}^{\lambda}$ 
&  Certain class of nonnegative, mutually equivalent functions,
see p.~\pageref{notat:Plam}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{P}^{\lambda\not\sim}$, $\widetilde{\mathcal{P}}^{\lambda}$ 
&  Certain classes of nonnegative functions,
see p.~\pageref{notat:Plamnotsim}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathcal{P}_{\Theta}^{\lambda}$, $\mathcal{P}_{emp}^{\lambda\perp}$, $\mathcal{P}_{\Theta,emp}^{\lambda}$ 
 &  Certain classes of nonnegative functions, 
see p.~\pageref{notat:Plamtheta}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\mathfrak{D}^{0}$, 
$\mathfrak{D}^{1}$, 
$\rho_{\mathbbm{Q}}$ 
& Functionals and mapping for decomposable pseudo-divergences,
\\
&  see Definition \ref{brostu2:def.dec} on p.~\pageref{brostu2:def.dec}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\psi^{dec}$, $\psi^{0}$, $\psi^{1}$, $\rho$
& Mappings for pointwise decomposable pseudo-divergences,
\\
&  see Definition \ref{brostu2:def.dec} on p.~\pageref{brostu2:def.dec}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$\psi_{m}^{dec}$ &  Perspective function of $\psi^{dec}$, see \eqref{27newbwithm}
\end{longtable}
}
...
\subsection*{New divergence toolkit}
...
($\sigma$-algebra) and a $\sigma$-finite measure $\lambda$ \label{notat:lambda}
(e.g. 
...
To address the entire functions as objects
we write $P := \big\{p(x)\big\}_{x \in \mathcal{X}}$, \label{notat:Pfunc}
$Q := \big\{q(x)\big\}_{x \in \mathcal{X}}$ \label{notat:Qfunc}
and alternatively sometimes also $p(\cdot)$, $q(\cdot)$.
To better highlight
...
we use the notation 
$\mathbbm{\overrightharp{P}}$,  \label{notat:overrightharpP}
$\mathbbm{\overrightharp{p}}$,
$\mathbbm{\overrightharp{Q}}$,
$\mathbbm{\overrightharp{q}}$
instead of $P$, $p$, $Q$, $q$ (where \overrightharp{$\cdots$} symbolizes a lying
$1$).
For instance, if $\lambda = \lambda_{L}$ \label{notat:lambdaL} is
...
and 
$\lambda := \lambda_{\#}$ \label{notat:lambdahash}
...
\label{notat:phi}  \label{notat:Mfunc1}
defined by 
...
$x \in \mathcal{X}$
\footnote{
as an example, let $\mathcal{X}= \mathbb{R}$, $\lambda = \lambda_{L}$ \label{notat:lambdaL2}
...
and $\mathbbm{M_{i}}$, $\mathbbm{m_{i}}(\cdot)$ 
when they are nonnegative
as well as 
$\mathbbm{\overrightharp{M}}_{i}$,
$\mathbbm{\overrightharp{m}}_{i}(\cdot)$ 
...
there holds \\
$D^{c}_{\phi,M_{1},M_{2},\mathbbm{M}_{3},\lambda}(P,Q) \geq 0$
with equality if
...
\begin{eqnarray} 
& & \hspace{-0.2cm}
\textit{(2)} \  D^{c}_{\phi,M_{1},M_{2},\mathbbm{M}_{3},\lambda}(P,Q) = 0 \ \ 
...
\noindent
Theorem \ref{brostu2:thm.2} -- whose proof
...
We continue with the inspection of interesting special cases of\\
...
at $\eta = \frac{Q}{M_{2}}$.
If one has a ``nonnegativity-setup'' (NN0) \label{notat:NN0}
in the sense that
...
\nonumber \\
& & \hspace{7.2cm} \phi_{\alpha}(\infty) := \lim_{t\uparrow \infty} \phi_{\alpha}(t)= \infty,
...
\\
& & \hspace{-0.3cm}   \textstyle
\phi_{\alpha}^{\prime}(\infty) \negthinspace := \negthinspace 
\lim_{t\uparrow \infty}\phi_{\alpha}^{\prime}(t) \negthinspace = \negthinspace
...
\\ 
& & \hspace{-0.2cm}   \textstyle 
\phi_{1}(t) \negthinspace := \negthinspace \lim_{\alpha \rightarrow 1} \phi_{\alpha}(t) 
\negthinspace = \negthinspace
...
\\ 
& & \hspace{-0.2cm}   \textstyle 
\phi_{0}(t) \negthinspace := \negthinspace \lim_{\alpha \rightarrow 0} \phi_{\alpha}(t) 
\negthinspace = \negthinspace
...
\\
& & \hspace{-0.2cm}   \textstyle
\psi_{\phi_{ie}}(0,t) \negthinspace = \negthinspace \lim_{s \rightarrow 0} \psi_{\phi_{ie}}(s,t) 
\negthinspace = \negthinspace \frac{1}{3} \cdot \boldsymbol{1}_{[1,\infty[}(t) + 
\frac{1}{3} \negthinspace \cdot \negthinspace \Big[1-(1-t)^{2} \cdot (1-2t)  \Big] \cdot \boldsymbol{1}_{]0,1[}(t) , 
\nonumber 
\\
& & \hspace{-0.2cm}   \textstyle
...
%
\noindent
\textbf{3.3.1 \  \label{notat:Subs.3.3.1}
...
\begin{eqnarray} 
& & \hspace{-0.4cm}   \textstyle \textstyle
0 \leq D^{c}_{\phi,M,M,R\cdot M,\lambda}(P,Q) 
\nonumber\\ 
& & \hspace{-0.4cm}   \textstyle 
\negthinspace : = \negthinspace
\olint_{{\mathcal{X}}} 
\Big[ \phi  \big( {
...
\Big] \negthinspace \cdot \negthinspace 
m(x) \negthinspace \cdot \negthinspace r(x) \, \mathrm{d}\lambda(x) \ , \ \ \ \  
...
(a) \ If one has a ``nonnegativity-setup'' (NN1) \label{notat:NN1}
in the sense that
...
\noindent
\footnote{as a side remark, let us mention here that in the special case 
of continuously differentiable \textit{strictly log-convex} divergence generator $\phi$,
one can construct divergences which are tighter than \eqref{brostu2:fo.def.40var}
respectively \eqref{brostu2:fo.def.41},
see Stummer \& Ki{\ss}linger~\cite{Stu:17a}; in a finite discrete space
and for differentiable \textit{exponentially concave} divergence generator $\phi$,
a similar tightening (called L-divergence) can be found in
Pal \& Wong~\cite{Pal:16,Pal:18} 
}.
...
\mathbbm{r}(x)
\negthinspace \cdot \negthinspace \big[ 
\frac{\mathbbm{p}(x)^{\alpha}}{\alpha \negthinspace \cdot \negthinspace (\alpha-1)} 
\negthinspace \cdot \negthinspace \boldsymbol{1}_{]1,\infty[}(\alpha) 
\negthinspace + \negthinspace \infty \negthinspace \cdot \negthinspace \boldsymbol{1}_{]-\infty,0[ \cup ]0,1[}(\alpha) 
\big] \negthinspace
\cdot \negthinspace \boldsymbol{1}_{]0,\infty[}\big(\mathbbm{p}(x) 
\big) \negthinspace
\cdot \negthinspace \boldsymbol{1}_{\{0\}}\big(\mathbbm{q}(x)\big)
\, \mathrm{d}\lambda(x)
\nonumber \\ 
& & \hspace{-0.2cm}   \textstyle 
+ \negthinspace
...
\mathbbm{r}(x) \negthinspace
\cdot \negthinspace \big[ 
\frac{\mathbbm{q}(x)^{\alpha}}{\alpha} \negthinspace \cdot \negthinspace \boldsymbol{1}_{]0,1[ \cup ]1,\infty[}(\alpha)
\negthinspace + \negthinspace \infty \negthinspace \cdot \negthinspace \boldsymbol{1}_{]-\infty,0[}(\alpha)
\big] \negthinspace
\cdot \negthinspace \boldsymbol{1}_{]0,\infty[}\big(\mathbbm{q}(x) 
\big) \negthinspace
\cdot \negthinspace \boldsymbol{1}_{\{0\}}\big(\mathbbm{p}(x)\big) 
\, \mathrm{d}\lambda(x) \, ,
\nonumber \\ 
& & \hspace{8.2cm} 
...
\nonumber \\ 
& & \hspace{-0.2cm}   \textstyle 
= \negthinspace \int_{{\mathcal{X}}} \negthinspace
\mathbbm{r}(x) \negthinspace \cdot \negthinspace 
\mathbbm{p}(x) \negthinspace \cdot \negthinspace \log\big(\frac{\mathbbm{p}(x)}{\mathbbm{q}(x)}\big)
\negthinspace \cdot \negthinspace \boldsymbol{1}_{]0,\infty[}\big(\mathbbm{p}(x) 
\negthinspace \cdot \negthinspace \mathbbm{q}(x)\big) \, \mathrm{d}\lambda(x) 
\negthinspace + \negthinspace \int_{{\mathcal{X}}} 
\mathbbm{r}(x) \negthinspace
\cdot \negthinspace (\mathbbm{q}(x) \negthinspace - \negthinspace \mathbbm{q}(x)) 
\, \mathrm{d}\lambda(x)  
\nonumber \\ 
& & \hspace{-0.2cm}   \textstyle 
+ \infty \cdot
...
\end{eqnarray}
\footnotetext{the first resp. second resp. third integral in \eqref{brostu2:fo.def.140a} can be interpreted 
as divergence-contribution of the function-(support-)overlap resp. of one part of the function-nonoverlap
(e.g. describing ``extreme outliers'') resp. of the other part of the function-nonoverlap 
(e.g. describing ``extreme inliers'')
}
...
``order$-\alpha$'' density-power divergences DPD \label{notat:DPD}
of Basu et al.~\cite{Bas:98}
...
is called 
``Kullback-Leibler information divergence'' between $\mathbbm{\overrightharp{P}}$ and $\mathbbm{\overrightharp{Q}}$,
...
which will become clear in the proof of Theorem \ref{brostu2:thm.2CASD} in the appendix). The analogue of 2(e)
...
$(\mathfrak{P}^{\mathbbm{R}\cdot \lambda}-\mathfrak{Q}^{\mathbbm{R}\cdot \lambda})[\mathcal{X}] 
= \int_{{\mathcal{X}}}  \big( \mathbbm{p}(x) - \mathbbm{q}(x) \big) 
\cdot \mathbbm{r}(x) \, \mathrm{d}\lambda(x) \in [0, \infty[$
(but not necessarily $\mathfrak{P}^{\mathbbm{R}\cdot \lambda}[\mathcal{X}] 
=  \int_{{\mathcal{X}}} \mathbbm{p}(x) \cdot \mathbbm{r}(x) \, \mathrm{d}\lambda(x) < \infty$,
$\mathfrak{Q}^{\mathbbm{R}\cdot \lambda}[\mathcal{X}] = \int_{{\mathcal{X}}} 
\mathbbm{q}(x) \cdot \mathbbm{r}(x) \, \mathrm{d}\lambda(x) < \infty$),
...
-- 
to the classical Csiszar-Ali-Silvey $\phi$-divergence CASD \label{notat:CASD}
...
\textit{(2)} \ \ D^{c}_{\phi,\mathbbm{Q},\mathbbm{Q},\mathbb{R}\cdot \mathbbm{Q},\lambda}(\mathbbm{P},\mathbbm{Q}) = 0 \ \ 
\textrm{if and only if} \ \  
\mathbbm{p}(x) = \mathbbm{q}(x) 
\ \textrm{for $\lambda$-a.a. $x \in \mathcal{X}$.} \quad \ 
...
\int_{{\mathcal{X}}} \negthinspace
\mathbbm{r}(x) \negthinspace \cdot \negthinspace \mathbbm{q}(x) \negthinspace \cdot \negthinspace \Big[ \big( {
\frac{\mathbbm{p}(x)}{\mathbbm{q}(x)}}\big)^{\alpha} \negthinspace - \negthinspace
\alpha \negthinspace \cdot \negthinspace \frac{\mathbbm{p}(x)}{\mathbbm{q}(x)} \negthinspace + \negthinspace \alpha -1 
\Big] 
\cdot \negthinspace \boldsymbol{1}_{]0,\infty[}\big(\mathbbm{p}(x)\negthinspace  \cdot \negthinspace \mathbbm{q}(x) 
\big)
\, \mathrm{d}\lambda(x) 
...
\negthinspace \mathbbm{r}(x) \negthinspace \cdot \negthinspace 
(\mathbbm{p}(x) \negthinspace - \negthinspace \mathbbm{q}(x)) \, \mathrm{d}\lambda(x)
\negthinspace + \negthinspace \infty \negthinspace \cdot \negthinspace 
\boldsymbol{1}_{]1,\infty[}(\alpha) \negthinspace \cdot \negthinspace
\int_{{\mathcal{X}}} \negthinspace
\negthinspace \mathbbm{r}(x) \negthinspace \cdot \negthinspace \mathbbm{p}(x) 
\negthinspace \cdot \negthinspace \boldsymbol{1}_{\{0\}}\big(\mathbbm{q}(x)\big)
\, \mathrm{d}\lambda(x)
...
\mathbbm{r}(x) \cdot \mathbbm{q}(x) 
\cdot \boldsymbol{1}_{\{0\}}\big(\mathbbm{p}(x)\big)
\, \mathrm{d}\lambda(x) , \footnotemark \qquad \ 
\nonumber  
\end{eqnarray}

\footnotetext{this can be interpreted analogously as 
in the footnote of \eqref{brostu2:fo.def.140a} }
\vspace{-0.2cm}
...
the divergence in \eqref{brostu2:fo.def.47b} simplifies to
...
-- respectively $\phi(1)=0$ and \eqref{brostu2:fo.AssuCASD1} 
-- turns into
...
=
\sum_{{x \in \mathcal{X}}}  \mathbbm{r}(x)  \cdot
 \mathbbm{q}(x) \cdot \phi  \big( {
\frac{ \mathbbm{p}(x)}{ \mathbbm{q}(x)}}\big)
\cdot \boldsymbol{1}_{]0,\infty[}\big(\mathbbm{p}(x) \cdot \mathbbm{q}(x) \big)
\quad \ 
...
\sum_{x \in \mathcal{X}}  
\mathbbm{r}(x) \negthinspace \cdot \negthinspace \mathbbm{q}(x) \negthinspace
\cdot \negthinspace\boldsymbol{1}_{\{0\}}\big(\mathbbm{p}(x)\big) . \quad \ 
...
\noindent
\textbf{
3.3.1.3  \ \label{notat:Subs.3.3.1.3}
...
\begin{eqnarray} 
& & \hspace{-0.3cm}   \textstyle \textstyle
...
\Big] 
\cdot w(p(x),q(x)) \cdot r(x) \, \mathrm{d}\lambda(x), \quad \ \ 
...
\begin{eqnarray}
& & \hspace{-0.2cm}   \textstyle \textstyle
0 \negthinspace \leq \negthinspace D^{c}_{\phi,W(P,Q),W(P,Q),R\cdot W(P,Q),\lambda_{\#}}\negthinspace(P,Q)  
\negthinspace = \negthinspace
\olsum_{{x \in \mathcal{X}}} 
\Big[ \phi  \big( {
\frac{p(x)}{w(p(x),q(x))}}\big) \negthinspace - \negthinspace\phi  \big( {\frac{q(x)}{w(p(x),q(x))}}\big)
\nonumber\\ 
& & \hspace{-0.2cm}   \textstyle  
- \phi_{+,c}^{\prime} 
\big( {\frac{q(x)}{w(p(x),q(x))}}\big) \cdot \big( \frac{p(x)}{w(p(x),q(x))}-\frac{q(x)}{w(p(x),q(x))}\big) 
...
the range $\mathcal{R}(Y_{1}, \ldots, Y_{N}) = \{ z_1, \ldots, z_s \}$ \label{notat:rangeRV}
...
\begin{eqnarray} 
& & \hspace{-0.2cm}   
1 \negthinspace = \negthinspace \int\displaylimits_{\mathcal{X}} 
\mathbbm{\overrightharp{p}}(x)
\, \mathrm{d}\lambda_{\#}(x)
\negthinspace = \negthinspace \int\displaylimits_{\mathcal{X}} 
\mathbbm{\overrightharp{p}}(x)
...
\eqref{brostu2:fo.Sing.1} to \eqref{brostu2:fo.Sing.3}
for $\lambda$-almost all $x \in \mathcal{X}$ --
which we denote by $\mathbbm{P} \perp \mathbbm{Q}$ \label{notat:perp}
and which basically amounts to pair of functions of the type
...
We say that a divergence (respectively dissimilarity respectively distance)
...
encom\-passing-concerning deficiency for
...
\negthinspace : = \negthinspace
\olint_{{\mathcal{X}}} 
\mathbbm{r}(x) \negthinspace  \cdot \negthinspace \Big[ \mathbbm{q}(x)^{\chi} \negthinspace \cdot \negthinspace \phi  \big( {
\frac{\mathbbm{p}(x)}{\mathbbm{q}(x)}}\big) 
\negthinspace - \negthinspace \mathbbm{q}(x)^{\chi} \negthinspace \cdot \negthinspace \phi  \big( 1 \big) 
\negthinspace - \negthinspace \phi_{+,c}^{\prime} 
\big( 1 \big) \negthinspace \cdot \negthinspace \big( \mathbbm{p}(x) 
\negthinspace \cdot \negthinspace \mathbbm{q}(x)^{\chi-1} \negthinspace - \negthinspace \mathbbm{q}(x)^{\chi} \big) 
\Big] \, \mathrm{d}\lambda(x)
\nonumber
\\ 
& & \hspace{-0.2cm}   \textstyle 
...
\nonumber\\ 
& & \hspace{-0.2cm}   \textstyle 
\negthinspace = \negthinspace \phi_{\chi}^{*}(0) \negthinspace \cdot \negthinspace
\int_{{\mathcal{X}}} 
\mathbbm{r}(x) \negthinspace \cdot \negthinspace \mathbbm{p}(x)^{\chi} 
\, \mathrm{d}\lambda(x)
\negthinspace + \negthinspace \big[ \phi(0) \negthinspace + \negthinspace \phi_{+,c}^{\prime}(1) 
\negthinspace - \negthinspace \phi(1) \big] \negthinspace \cdot \negthinspace
\int_{{\mathcal{X}}} 
\mathbbm{r}(x) \negthinspace \cdot \negthinspace \mathbbm{q}(x)^{\chi} 
\, \mathrm{d}\lambda(x)
  \,  \qquad \ \ 
...
\begin{eqnarray} 
& & \hspace{-0.2cm}   
\textit{(2)} \quad D^{c}_{\phi,\mathbbm{Q},\mathbbm{Q},\mathbb{R}\cdot \mathbbm{Q}^{\chi},\lambda}(\mathbbm{P},\mathbbm{Q}) = 0 \quad
...
\olint_{{\mathcal{X}}} 
\mathbbm{r}(x) \negthinspace  \cdot \negthinspace \Big[ \mathbbm{q}(x)^{\chi} 
\negthinspace \cdot \negthinspace \phi_{\alpha}  \big( {
\frac{\mathbbm{p}(x)}{\mathbbm{q}(x)}}\big) \negthinspace - \negthinspace \mathbbm{q}(x)^{\chi} 
\negthinspace \cdot \negthinspace \phi_{\alpha}  \big( 1 \big) 
\negthinspace - \negthinspace \phi_{\alpha}^{\prime} 
\big( 1 \big) \negthinspace \cdot \negthinspace \big( \mathbbm{p}(x) 
\negthinspace \cdot \negthinspace \mathbbm{q}(x)^{\chi-1} \negthinspace - \negthinspace \mathbbm{q}(x)^{\chi} \big) 
\Big]  \, \mathrm{d}\lambda(x)   
\nonumber\\
...
\lambda_{2}}_{\theta }[\bullet]: \theta \in \Theta \}$ \label{notat:Qthetalamda2}
of probability
measures on $({\mathcal{X}},{\mathcal{F}})$ indexed by a set of parameters 
$\Theta \subset {\mathbb{R}}^{d}$ \label{notat:Theta} 
(the non-parametric case works
...
$\mathfrak{\overrightharp{Q}}^{\mathbb{1}\cdot 
\lambda_{2}} \sim
\widetriangle{\mathfrak{\overrightharp{Q}}}^{\mathbb{1}\cdot 
...
$\widetriangle{\mathbbm{\overrightharp{q}}}(x) = 0$; this implies
in particular that 
$\mathbbm{\overrightharp{q}}(x) 
\cdot \boldsymbol{1}_{\{0\}}\big(\widetriangle{\mathbbm{\overrightharp{q}}}(x)\big) = 0$
and 
$\widetriangle{\mathbbm{\overrightharp{q}}}(x) 
...
$\mathbbm{\overrightharp{q}}(x) \cdot \widetriangle{\mathbbm{\overrightharp{q}}}(x) >0$
(and hence, $\boldsymbol{1}_{]0,\infty[}\big(\mathbbm{\overrightharp{q}}(x) \cdot \widetriangle{\mathbbm{\overrightharp{q}}}(x) 
\big) = 1$)
for $\lambda_{2}$-a.a. $x \in \mathcal{X}$.
Clearly, since any $\lambda_{2}$-aggregated divergence $D_{\lambda_{2}}(\cdot,\cdot)$  \label{notat:Dlambda}
satisfies
...
$\mathbbm{\overrightharp{Q}}_{\theta} := 
\big\{\mathbbm{\overrightharp{q}}_{\theta}(x)\big\}_{x \in \mathcal{X}}$ \label{notat:Qtheta}
...
(where $s$ is an integer larger than one or infinity), then
...
\begin{eqnarray}
& & \hspace{-0.2cm}   \textstyle
\widehat{\theta}_{N}(\omega) := \widehat{\theta}_{N,D_{\lambda_{\#}}}(\omega)
:= \arginf_{\theta \in \Theta} \ 
D_{\lambda_{\#}}\big(
\mathbbm{\overrightharp{P}}_{N}^{emp(\omega)},
\mathbbm{\overrightharp{Q}}_{\theta}
\big) 
\label{brostu2:fo.829}
\end{eqnarray}
...
$\mathbbm{\overrightharp{q}}_{\theta}(x) >0$ (i.e. an ``extreme inlier''), and hence,
...
\footnote{
e.g. applying the divergence \eqref{brostu2:fo.377} for $\alpha \in \mathbb{R}\backslash\{0,1\}$, the sum-entry 
$\mathbbm{r}(x) \cdot  
\frac{\mathbbm{\overrightharp{q}}_{\theta}(x)^{\alpha}}{\alpha} 
$
appears, which can be viewed as penalty for the cell $x$ being empty of data observations
(``intrinsic empty-cell-penalty''); for divergence \eqref{brostu2:fo.def.45cdis}, the penalty
is $\phi(0) \cdot \mathbbm{r}(x) \cdot \mathbbm{\overrightharp{q}}_{\theta}(x)$ 
}.
...
\begin{eqnarray} 
& & \hspace{-0.2cm}   \textstyle
0 \negthinspace = \negthinspace \mathfrak{\overrightharp{Q}}^{\mathbb{1}\cdot 
\lambda_{2}}_{\theta }[\{z\}] 
\negthinspace = \negthinspace Pr[Y \in \{z\} \, | \, \theta ] 
\negthinspace = \negthinspace \int_{\mathcal{X}} \boldsymbol{1}_{\{z\}}(x) 
\negthinspace \cdot \negthinspace \mathbbm{\overrightharp{q}}_{\theta}(x) \, \mathrm{d}\lambda_{2}(x)
\textrm{ \ for all $z\in \mathcal{X}$, $\theta \in \Theta$} 
\nonumber\\
...
\negthinspace = \negthinspace \int_{\mathcal{X}} \mathbbm{\overrightharp{q}}_{\theta}(x) \, \mathrm{d}\lambda_{2}(x) =1
\ \  \textrm{for all $\theta \in \Theta$} \qquad \  
...
$\widetilde{\mathbbm{\overrightharp{Q}}}_{\theta} := 
\big\{\widetilde{\mathbbm{\overrightharp{q}}}_{\theta}(x)\big\}_{x \in \mathcal{X}}$ \label{notat:Qtilde}
...
the data-observa\-tions $Y_{1}(\omega), \ldots, Y_{N}(\omega)$, and thus is 
...
$\mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)} \not\perp 
\mathbbm{\overrightharp{Q}}_{\theta}$ \label{notat:nonsing}
but 
...
+ \phi(0) \negthinspace \cdot \negthinspace \int_{{\mathcal{X}}} \mathbbm{\overrightharp{q}}_{\theta}(x)^{\chi} 
...
\item[(GR)] grouping (partitioning, quantization) of data: \  
convert\footnote{
in several situations, such a conversion can appear in a natural way;
e.g. an institution may generate/collect data of ``continuous value'' but 
mask them for external data analysts to group-frequencies, for reasons of confidentiality 
(information asymmetry)
}
everything into a purely discrete context, by
...
For the subcase of Csiszar-Ali-Slivey divergences and adjacently related divergences,
thorough statistical investigations (such as efficiency, robustness, types of grouping, 
grouping-error sensitivity, etc.) 
of the corresponding minimum-divergence-estimation can be found e.g. in  
Victoria-Feser \& Ronchetti~\cite{Vic:97},
Menendez et al.~\cite{Men:98,Men:01a,Men:01b},
Morales et al.~\cite{Mor:04,Mor:06},
Lin \& He~\cite{Lin:06}. 
...
For the subcase of Csiszar-Ali-Slivey divergences,
thorough statistical investigations (such as efficiency, robustness, information loss, etc.) of the corresponding
minimum-divergence-estimation can be found e.g. in 
Basu \& Lindsay~\cite{Bas:94}, Park \& Basu~\cite{Par:04},
Chapter 3 of Basu et al.~\cite{Bas:11},
Kuchibhotla \& Basu~\cite{Kuc:15},  
Al Mohamad~\cite{AlM:16},
and the references therein.
Due to the ``curse of
...
For instance (in preparation for divergence valuation), take $\mathcal{X}=\mathbb{R}^d$, $\lambda_{2}$ to be the $d-$dimensional
Lebesgue measure and
$\mathbbm{\overrightharp{p}}_{N}^{emp,smo}(x) := \frac{1}{N} \sum_{i=1}^{N} K(x,Y_i,h_{n})
= \int_{\mathcal{X}}  K(x,y,h_{n}) \, \mathrm{d} \mathfrak{\overrightharp{P}}_{N}^{emp}(y)
$ where $K(\cdot,\cdot,\cdot)$ is an appropriate smooth kernel function
with ``bandwidth'' $h_{n}$, e.g. $K(x,y,h_{n}) := \frac{1}{h_{n}} \widetriangle{K}\left(\frac{x-y}{h_{n}} \right)$
with appropriate nonnegative function $\widetriangle{K}(\cdot)$ satisfying $\int_{\mathbb{R}^d} \widetriangle{K}(y) 
\, \mathrm{d} \lambda_{2}(y) =1$. Since such kernel smoothers KS use local averaging,
and for large $d$ most neighborhoods tend to be empty of data observations (because data often ``live''
on lower-dimensional manifolds, sparsity of data), 
a typical KS technique (choosing concrete kernels and bandwidths, etc.) 
needs then a huge amount $N$ of data to provide a
reasonable accuracy; for $d=8$ one may need $N$ to be 1 million.
For background details, the reader is e.g. referred to~\cite{Sco:91},~\cite{Das:00},~Chapter 7 of \cite{Sco:15}
and the references therein.

\end{enumerate}
For the sake of brevity, a detailed discussion of (GR) and (SM) is beyond the scope of
this paper.
...
use henceforth the 
notations 
$\mathcal{P}^{\lambda}$ \label{notat:Plam}
...
and $\mathcal{P}^{\lambda\not\sim}$ \label{notat:Plamnotsim}
for a corresponding class of nonnegative
(not necessarily mutually equivalent) functions 
such that $\mathbbm{P}_{1} \not\sim \mathbbm{P}_{2}$ \label{notat:notsim}
for all 
$\mathbbm{P}_{1} \in \mathcal{P}$, $\mathbbm{P}_{2} \in \mathcal{P}^{\lambda\not\sim}$.
Furthermore, we employ 
$\widetilde{\mathcal{P}}^{\lambda} := \mathcal{P}^{\lambda} \cup \mathcal{P}^{\lambda\not\sim}$ \label{notat:Plamtil}
and specify:
...
\mathcal{P}_{\Theta}^{\lambda} \cup \mathcal{P}_{emp}^{\lambda\perp}$; \label{notat:Plamthetaemp}
...
\mathfrak{D}^{0}(\mathbbm{P})+\mathfrak{D}^{1}(\mathbbm{Q})
+\int_{\mathcal{X}} \rho_{\mathbbm{Q}}(x) \cdot \mathbbm{p}(x) \, \mathrm{d}\lambda(x)
\ \  \text{for all }\mathbbm{P} \in \widetilde{\mathcal{P}}^{\lambda},
\mathbbm{Q} \in \mathcal{P}^{\lambda}. \nonumber \\  
\label{26anew}
\end{eqnarray} 
...
$D_{\lambda}(\mathbbm{P},\mathbbm{Q}) = 
\int_\mathcal{X} \psi^{dec}(\mathbbm{p}(x), \mathbbm{q}(x))  \, \mathrm{d}\lambda(x)$ 
...
\begin{eqnarray} 
& & \hspace{-0.3cm}   \textstyle 
\mathbb{R} \ni \psi^{dec}(s,0) \negthinspace = \negthinspace \psi^{0}(s+h_{0}(x,s)) 
\negthinspace \cdot \negthinspace \widecheck{c_{0}}
\negthinspace + \negthinspace \psi^{1}(h_{1}(x)) \cdot \widecheck{c_{1}} 
\negthinspace + \negthinspace \rho(h_{2}(x)) \negthinspace \cdot \negthinspace s  \   
\textrm{for all $s >0$}, 
\qquad \ 
\label{27newe}
\\
...
\negthinspace = \negthinspace \arginf_{\theta \in \Theta} \negthinspace \big[ 
\mathfrak{D}^{1}(\mathbbm{\overrightharp{Q}}_{\theta})
\negthinspace + \negthinspace 
\int_{\mathcal{X}} \rho_{\mathbbm{\overrightharp{Q}}_{\theta}}(x) \negthinspace \cdot \negthinspace 
\mathbbm{\overrightharp{p}}(x) \, \mathrm{d}\lambda(x) \big]
\ \  \text{for all }\mathbbm{\overrightharp{P}} \in 
\mathcal{P}_{\Theta,emp}^{\lambda}. \qquad \  
\label{c1newb}
\end{eqnarray}
...
statistics-ML-AI 
context 
we have chosen $\lambda[\bullet] := \lambda_{1}[\bullet] + \lambda_{2}[\bullet]$ with\\
 $\lambda_{1}[\bullet] := \sum_{z \in \mathcal{X}} \boldsymbol{1}_{\mathcal{R}(Y_{1}(\omega), \ldots, Y_{N}(\omega))}(z) 
...
\\
& & \hspace{-0.2cm}   \textstyle
+ \frac{1}{N} \negthinspace \sum_{i=1}^{N}  \negthinspace
\Big[
\frac{\Big(\mathbbm{\overrightharp{q}}_{\theta}(Y_{i}(\omega))\Big)^{\alpha}}{
\alpha  \cdot  \mathbbm{\overrightharp{p}}_{N}^{emp(\omega)}(Y_{i}(\omega))}
\negthinspace - \negthinspace \frac{\Big(\mathbbm{\overrightharp{q}}_{\theta}(Y_{i}(\omega))\Big)^{\alpha-1}}{\alpha-1}
\Big]
\negthinspace + \negthinspace \frac{1}{N} \sum_{i=1}^{N}  
...
& & \hspace{-0.2cm}   
D_{\lambda,1}\Big(\mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)},\widetilde{\mathbbm{\overrightharp{Q}}}_{\theta}\Big) 
\negthinspace = \negthinspace \frac{1}{N} \sum_{i=1}^{N} \log\Big(\mathbbm{\overrightharp{p}}_{N}^{emp(\omega)}(Y_{i}(\omega))\Big) 
\negthinspace - \negthinspace \frac{1}{N} \sum_{i=1}^{N} \log\Big(\mathbbm{\overrightharp{q}}_{\theta}(Y_{i}(\omega))\Big), 
\qquad \
...
\begin{eqnarray} 
& & \hspace{-0.2cm}  \textstyle 
0 \negthinspace < \negthinspace D_{\lambda,1}\Big(\mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)},\widetilde{\mathbbm{\overrightharp{Q}}}_{\theta}\Big) 
\negthinspace = \negthinspace \frac{1}{N} \negthinspace \sum_{i=1}^{N} \negthinspace
\log\Big(\mathbbm{\overrightharp{p}}_{N}^{emp(\omega)}(Y_{i}(\omega))\Big) 
\negthinspace - \negthinspace \frac{1}{N} \negthinspace 
\sum_{i=1}^{N} \negthinspace \log\Big(\mathbbm{\overrightharp{q}}_{\theta}(Y_{i}(\omega))\Big) 
\nonumber
\\
& & \hspace{-0.2cm}   \textstyle
+ \frac{1}{N} \sum_{i=1}^{N}  
...
\\
& & \hspace{-0.2cm}   \textstyle
\negthinspace = \negthinspace \int_{\mathcal{X}} \negthinspace \psi^{1}\negthinspace\Big(\frac{\mathbbm{\overrightharp{q}}_{\theta}(x)}{\mathbbm{\overrightharp{q}}_{\tau}(x)}\Big)  
\negthinspace \cdot \negthinspace \mathbbm{\overrightharp{q}}_{\tau}(x) \, \mathrm{d}\lambda_{2}(x)
\negthinspace + \negthinspace \sum_{x \in \mathcal{X}}  
\negthinspace \Big[\mathbbm{\overrightharp{q}}_{\tau}(x) \cdot 
\psi^{0}\negthinspace\Big(\frac{\mathbbm{\overrightharp{p}}_{N}^{\overline{emp}(\omega)}(x)}{\mathbbm{\overrightharp{q}}_{\tau}(x)}
\negthinspace + \negthinspace h_{0}\Big(x,\frac{\mathbbm{\overrightharp{p}}_{N}^{\overline{emp}(\omega)}(x)}{\mathbbm{\overrightharp{q}}_{\tau}(x)}
\Big)\Big) \negthinspace \cdot \negthinspace \widecheck{c_{0}}
\nonumber
\\
& & \hspace{-0.2cm}   \textstyle
...
$0 \negthinspace = \negthinspace \min_{\theta\in \Theta} \min_{\tau \in \Theta} D_{\phi,\mathbbm{\overrightharp{Q}}_{\tau},\lambda_{2}}(\mathbbm{\overrightharp{Q}}_{\theta_{0}},\mathbbm{\overrightharp{Q}}_{\theta})$, 
$\theta_{0} \negthinspace = \negthinspace \argmin_{\theta\in \Theta} \min_{\tau \in \Theta} D_{\phi,\mathbbm{\overrightharp{Q}}_{\tau},\lambda_{2}}(\mathbbm{\overrightharp{Q}}_{\theta_{0}},\mathbbm{\overrightharp{Q}}_{\theta})$,
and in case of $\max_{\tau \in \Theta} \negthinspace D_{\phi,\mathbbm{\overrightharp{Q}}_{\tau},\lambda_{2}}\negthinspace \negthinspace 
(\mathbbm{\overrightharp{Q}}_{\theta_{0}},\mathbbm{\overrightharp{Q}}_{\theta}) 
\negthinspace < \negthinspace \infty$
also 
$0 \negthinspace = \negthinspace \min_{\theta\in \Theta} \max_{\tau \in \Theta} \negthinspace D_{\phi,\mathbbm{\overrightharp{Q}}_{\tau},\lambda_{2}}\negthinspace \negthinspace
(\mathbbm{\overrightharp{Q}}_{\theta_{0}},\mathbbm{\overrightharp{Q}}_{\theta})$, 
...
\negthinspace \cdot \negthinspace \big( \frac{\mathbbm{\overrightharp{q}}_{\theta_{0}}(x)}{\mathbbm{\overrightharp{q}}_{\theta}(x)}
\negthinspace -  \negthinspace \frac{\mathbbm{\overrightharp{q}}_{\tau}(x)}{\mathbbm{\overrightharp{q}}_{\theta}(x)}\big) 
\Big] \negthinspace \cdot \negthinspace
...
\\
& & \hspace{-0.2cm}   \textstyle
+ \rho_{\mathbbm{\overrightharp{q}}_{\tau}(x)}^{sub}\negthinspace\Big(
\mathbbm{\overrightharp{q}}(x) \negthinspace + \negthinspace h_{2}(x) \negthinspace \cdot \negthinspace 
\boldsymbol{1}_{\{0\}}(\mathbbm{\overrightharp{q}}(x))
\Big) \negthinspace \cdot \negthinspace 
\mathbbm{\overrightharp{p}}(x) 
\Big] 
  \, \mathrm{d}\lambda(x) \, \ \ 
...
\sup_{\tau \in \Theta} 
\mathcal{D}^{c}_{\phi,\mathbbm{\overrightharp{Q}}_{\tau},\lambda}\Big(\mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)},\widetilde{\mathbbm{\overrightharp{Q}}}_{\theta}\Big) 
\ \  \textrm{for} \ \ \mathbbm{\overrightharp{P}}_{N}^{\overline{emp}(\omega)} \in 
\mathcal{P}_{emp}^{\lambda\perp} \, , 
\nonumber\\
\ 
\label{brostu2:fo.941}
\end{eqnarray}
...
has been considered in Toma \& Broniatowski~\cite{Tom:11}, Al Mohamad~\cite{AlM:16}, for Csiszar-Ali-Silvey divergences.
\end{remark}
...
Motivated by fields of applications from statistics, machine learning, artificial
intelligence and information geometry,  
we presented for a wide audience a new unifying framework of divergences between functions. 
Within this, we 
illuminated several important subcases -- such as scaled Bregman divergences 
and Csiszar-Ali-Silvey $\phi$-divergences -- as well as involved subtleties and pitfalls.
For the often desired task of finding the ``continuous'' model
with best divergence-proximity to the observed ``discrete'' data, 
we summarized existing and also derived new approaches.
As far as potential future studies is concerned, the kind of universal nature
of our introduced toolkit suggests quite a few possibilities 
for further adjacent developments and concrete applications.
...
\vspace{0.5cm}
\noindent
\textbf{Acknowledgement:} We are 
%very 
grateful to three anonymous referees for their 
very useful 
suggestions and comments.
W. Stummer wants to thank very much the Sorbonne Universite
...
0 \negthinspace = \negthinspace \int_{{\mathcal{X}}} \negthinspace
\big( \mathbbm{p}(x) \negthinspace - \negthinspace 
\mathbbm{q}(x) \big) \cdot \mathbbm{r}(x) \, \mathrm{d}\lambda(x) \negthinspace = \negthinspace
\int_{{\mathcal{X}}} \negthinspace 
\big( \mathbbm{p}(x) - \mathbbm{q}(x) \big)  
\negthinspace \cdot \negthinspace\boldsymbol{1}_{]\mathbbm{q}(x),\infty[}\big(\mathbbm{p}(x)\big)
\negthinspace \cdot \negthinspace \mathbbm{r}(x)
\, \mathrm{d}\lambda(x) \qquad \ \ 
\nonumber  
\end{eqnarray}
...
\bibitem{Ama:18}
Amari, S.-I., Karakida, R., Oizumi, M.:
Information geometry connecting Wasserstein distance
and Kullback-Leibler divergence via the entropy-relaxed
transportation problem.
Info. Geo. (2018), 
https://doi.org/10.1007/s41884-018-0002-8.
...
\bibitem{Bas:94} 
Basu, A., Lindsay, B.G.:
Minimum disparity estimation for continuous models:
efficiency, distributions and robustness.
Ann. Inst. Statist. Math. 46(4), 683--705 (1994)
...
Discr. Comput. Geometry 44(2), 281–-307 (2010)
...
J. Multiv. Anal. 100(1), 16--36 (2009)
...
Calcutta Statist. Assoc. Bull. 50(3-4), 157--178 (2000)
...
To appear in \textit{Applied Stochastic Models in Business and Industry} (2018)

\bibitem{Kuc:15}
Kuchibhotla, A.K., Basu, A.:  
A general setup for minimum disparity estimation. 
Statist. Probab. Lett. 96: 68–-74 (2015)
...
clustering.  
IEEE Trans. Pattern Anal. Mach. Intell. 34(12),
2407--2419 (2012) 
...
Comm. Statist. - Theory Methods 27(3),
609--633 (1998) 

\bibitem{Men:01a}
Menendez, M., Morales, D., Pardo, L., Vajda, I.: 
Minimum divergence estimators based on grouped data.  
Ann. Inst. Statist. Math. 53(2),
277--288 (2001) 
...
In: Lopez-Diaz, M., et al. (eds.) Soft Methodology and
...
Comm. Statist. - Theory Methods 35(9),
1629--1653 (2006)
...
IEEE Trans. Inform. Theory 62(1), 527--538 (2016)
...
Math. Finan. Econon. 10, 263--293 (2016)

\bibitem{Pal:18}
Pal, S., Wong, T.-K. \negthinspace L.:
Exponentially concave functions and a new information geometry. 
Ann. Probab. 46(2), 1070--1113 (2018)
...
\bibitem{Par:04}
Park, C., Basu, A.:
Minimum disparity estimation: asymptotic normality
and breakdown point results.
Bull. Inform. Kybernetics 36, 19--33 (2004) 
...
applicability in statistics and information theory. 
Statistics 44, 169–-187 (2010)

\bibitem{Stu:12}
Stummer, W., Vajda, I.: 
On Bregman distances and divergences of probability measures.
IEEE Trans. Inform. Theory 58(3), 1277--1288 (2012)
...
\bibitem{Vic:97}
Victoria-Feser, M.-P., Ronchetti, E.:  
Robust estimation for grouped data. 
J. Amer. Statist. Assoc. 92(437), 333--340 (1997)
...
IEEE Communic. Surveys \& Tutorials 17(1), 
70--91 (2015)
...
IEEE Trans. Knowledge Data Engin. 24(3), 
478--491 (2012)
