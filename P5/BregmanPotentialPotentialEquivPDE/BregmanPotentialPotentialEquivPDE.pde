...
double gradF0(double theta) {
  return  -1.0/theta;
}

double gradG0(double eta) {
  return  -1.0/eta;
}


double G0(double eta) {
  return  eta*gradG0(eta)-F0(gradG0(eta));
}
...
Line tangentF=new Line(gradF(theta0),F(theta0)-theta0*gradF(theta0));
 ...
 Line tangentF0=new Line(gradF0(p0),F0(p0)-p0*gradF0(p0));
 
 stroke(255,0,0);
 line(bb.x2X(bb.minx), bb.y2Y(tangentF0.x2y(bb.minx)), bb.x2X(bb.maxx), bb.y2Y(tangentF0.x2y(bb.maxx)) );
 
  line(bb.x2X(p0), bb.y2Y(0), bb.x2X(p0), bb.y2Y(tangentF0.x2y(p0)) );
  
  //double w0=0.25*eta0*eta0-theta0*eta0+F(theta0);
  
  double w0=-theta0*gradF(theta0)+F(theta0)+p0*gradF0(p0)-F0(p0);
  println("w0="+w0);
  
  
  
  stroke(0,0,0);
  Line TtangentF0=new Line(gradF0(p0),F0(p0)-p0*gradF0(p0)+w0);
  
  strokeWeight(2);
 line(bb.x2X(bb.minx), bb.y2Y(TtangentF0.x2y(bb.minx)), bb.x2X(bb.maxx), bb.y2Y(TtangentF0.x2y(bb.maxx)) );
   strokeWeight(1);
