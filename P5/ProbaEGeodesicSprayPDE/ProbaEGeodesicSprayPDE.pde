...
void drawFullFisherRaoGeodesic(double alpha, double []p, double [] q)
{
  
}

void drawFullGeodesic(double alpha, double []p, double [] q)
{
/*
  if (alpha==-1) 
  {
...
double tmax=50;
if (alpha==0) {tmax=500;tstep=0.5;}

  for (t=-tmax; t<tmax; t+=tstep)
  {
    double [] interp=geodesic(alpha, p, q, t);
     double [] interpt=geodesic(alpha, p, q, t+tstep);
    //MyPoint(interp[0], interp[1]);
    
    if ((interp[0]>0)&&(interp[1]>0)&&(interpt[0]>0)&&(interpt[1]>0)&&(interp[0]+interp[1]<1)&&(interpt[0]+interpt[1]<1))
...
double alpha;
  ...
  surface.setTitle("Exponential (red), mixture (blue) and Fisher-Rao (magenta) geodesics");
...
void drawVoronoi()
{
   surface.setTitle("e-geodesic spray from simplex vretex");
  background(255, 255, 255);
  stroke(0);
...
// simplex board
  ...
  double angle; 
  double tangle=0.05; //0.1
  for(angle=-Math.PI;angle<Math.PI;angle+=tangle)
  {
    
    pp[0]=p[0]+0.1*Math.cos(angle);
     pp[1]=p[1]+0.1*Math.sin(angle);
  drawFullGeodesic(galpha, p, pp);
  
  }
  
  
  stroke(255,0,0);
   drawFullGeodesic(galpha, p, q);
  
}

void drawAlpha(double alpha)
{
   surface.setTitle("e-geodesic spray in the probability simplex");
  background(255, 255, 255);
  stroke(0);
...
// simplex board
 ...
 // MyPoint(q[0], q[1]);
  ...
  double angle; 
  double tangle=0.2; //0.1
  for(angle=-Math.PI;angle<Math.PI;angle+=tangle)
  {
    
    pp[0]=p[0]+0.1*Math.cos(angle);
     pp[1]=p[1]+0.1*Math.sin(angle);
  drawFullGeodesic(alpha, p, pp);
  
  }
  
  
   stroke(0);
 // drawFullGeodesic(alpha, p, q);
  
   strokeWeight(2);
  stroke(0);
  
  /*
  MyLine(V[0][0], V[0][1], p[0], p[1]);
  MyLine(V[1][0], V[1][1], p[0], p[1]);
  MyLine(V[2][0], V[2][1], p[0], p[1]);
  */
  
  /*
    drawFullGeodesic(alpha,V[0], p);
     drawFullGeodesic(1,V[1], p);
        drawFullGeodesic(1,V[2], p);
  */
  strokeWeight(1);
...
void initialize()
{
  ...
  if (toggleInitDegenerate)   initializeDegenerate();
  else{
  //double [][] V;
  V=new double [3][2];
  V[0][0]=0;
