...
void setup()
{
  System.out.println("Testing Legendre invariance for the multivariate case");
...
void TestLegendre()
{
  ...
  b=new Matrix(2, 1);
  for (i=0; i<2; i++) b.set(i, 0, Math.random());
  
  c=new Matrix(2, 1);
  for (i=0; i<2; i++) c.set(i, 0, Math.random());
  ...
  t1=new Matrix(2, 1);
  t1.set(0, 0, Math.log(p1[0]/p1[2]));
  t1.set(1, 0, Math.log(p1[1]/p1[2]));
  t2=new Matrix(2, 1);
  t2.set(0, 0, Math.log(p2[0]/p2[2]));
  t2.set(1, 0, Math.log(p2[1]/p2[2]));
...
double Fbar(Matrix t) {
  return lambda*F(A.times(t).plus(b))+Inner(c, t)+d;
}

Matrix gradFbar(Matrix t) {
  return (A.transpose()).times(lambda).times(gradF(A.times(t).plus(b))).plus(c);
}
...
double Gbar(Matrix e) {
  return Inner(e, gradGbar(e))-Fbar(gradGbar(e));
}
