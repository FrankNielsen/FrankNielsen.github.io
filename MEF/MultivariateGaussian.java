...
public final class MultivariateGaussian extends ExponentialFamily<PVector, PVectorMatrix>{	
	...
	public double density(PVector x, PVectorMatrix param){
		if (param.type==TYPE.SOURCE_PARAMETER){
			double mah = (x.Minus(param.v)).InnerProduct(param.M.Inverse().MultiplyVectorRight(x.Minus(param.v)));
			double res = Math.exp(-0.5d*( mah ));
			return res / (Math.pow(2.0d*Math.PI, (double)x.dim/2.0d)*Math.sqrt(param.M.Determinant()));
		}
