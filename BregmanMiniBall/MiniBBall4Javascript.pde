...
// Frank Note:
/* 
Javascript does not allow to have a method declared several times with
the same number of args (of different types), so we changes de PointSet class 
adding dummy args.
*/
...
private PointSet dataset= new PointSet(maxcard,DF,0,0);
...
void keyPressed()
{
  if (key=='a') {// add a new point
 		...
 		dataset=new PointSet(dataset,new Point(xx,yy),0);
 	}
...
void initSet()
{
  dataset=new PointSet(maxcard, DF,0,0);
...
void setup(){
	...
	frameRate(20);
	size(400,400); // Can't use w and h here!
	maxcard=30;
	w=400;
    h=400;
	int i;
...
void draw(){
	...
	text("Center point: ("+BD.center.x+" "+BD.center.y+") −  Radius:"+BD.rad, xpad, h-hmenu+31);
...
class PointSet
{
	...
	//
	PointSet(PointSet another, Point p, int uselessArg)
	{
		int i;
	...
	//
	PointSet(int card, BregmanDivergence BDiv, int uselessArg, int uselessArg2)
	{
		int i;
		double xx,yy;
...
/*
		...
		print("\n");
		*/
		radiusball=BDiv.divergence(centerball, new Point(0.1,0.1));
		...
		for(i=0;i<n;i++)
		{
			...
			while ( BDiv.divergence( centerball, drawpoint ) > radiusball )
			{
				drawpoint.x=random(1.0);
				drawpoint.y=random(1.0);
			}
	...
	BregmanDisk MiniDisc(PointSet set, PointSet basis)
	{
		...
		if (n+b>2)
		{
			...
			if (FallOutside(result,set.array[0]))
			{
				...
				PointSet newbasis=new PointSet(basis,set.array[0],0);
