...
the equivalent generator $G(\theta):=-\log p_\theta(x)=F(\theta)-\inner{t(x)}{\theta}-k(x)$ (i.e., $a=-t(x)$) and $b=-k(x)$, and express the Kullback-Leibler divergence, the  skewed Bhattacharrya divergences, the $\alpha$-divergences and many other statistical distances between densities of a natural exponential family $\left\{p_\theta(x)=1_{\calX}(x)\ \exp\left(\inner{t(x)}{\theta}-F(\theta)+k(x)\right)\right\}$ without {\it explicitly} using the log-normalizer $F(\theta)=\log \left(\int_{x\in\calX} \exp\left(\inner{t(x)}{\theta}+k(x)\right)\dmu(x)\right)$ of the exponential family (also called cumulant function or log-partition function).
